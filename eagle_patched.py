...
class SpecDecodeBaseProposer:
    def __init__(
        ...
        if self.needs_extra_input_slots:
            ...
            # _raise_if_multimodal() removed: text-only drafters work with
            # multimodal targets — the drafter proposes text tokens while
            # the target handles multimodal embeddings independently.
    ...
    def validate_same_kv_cache_group(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        Relaxed for models with heterogeneous attention (e.g. Gemma 4)
        which use multiple KV cache groups (sliding window + global).
        """
        ...
        if len(draft_gids) > 1:
            logger.info(
                "Draft model spans %d KV cache groups (heterogeneous "
                "attention). Using first group for common metadata.",
                len(draft_gids),
            )
    ...
    def initialize_attn_backend(
        ...
        Supports drafters with multiple KV cache groups (heterogeneous
        attention like Gemma 4's mixed head_dim 256/512).
        """
        all_attn_layers = get_layers_from_vllm_config(
            self.vllm_config,
            AttentionLayerBase,  # type: ignore[type-abstract]
        ...
        # Build a map from layer_name -> (gid, kv_cache_spec)
        ...
        # Use the first matching group as the primary (for common metadata)
        if layer_to_group:
            self.kv_cache_gid = next(iter(layer_to_group.values()))[0]

        # Key attention groups by (backend_class, kv_cache_group_id) so
        # layers in different KV cache groups get separate AttentionGroups
        # even when they share the same backend class.
